...
| **Документы** | `/documents` | реестр файлов + вкладка **Шаблоны** (каталог типов и конструктор шапки/примечаний) | скачать повторно; настроить шаблон |
...
На `/documents` → **Шаблоны** — каталог пяти системных типов (`estimate_internal`, `estimate_client`, `estimate_client_priced`, `contract`, `technichka`) с описанием полей CRM, образцами Word/PDF и конструктором MVP: логотип/блоки, заголовок и примечания с плейсхолдерами `{{deal.title}}`, `{{company.name}}`, `{{manager.name}}` и т.д. Таблица позиций пока фиксирована кодом.
